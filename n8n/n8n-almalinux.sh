...
clean_previous_installation() {
    ...
    # Xoa cau hinh Nginx
    if [ -f "/etc/nginx/conf.d/n8n.conf" ]; then
...
install_dependencies() {
    ...
    # Bat buoc phai co EPEL de cai certbot
    echo -e "${CYAN}Cai dat kho EPEL...${NC}"
    sudo dnf install -y epel-release
    check_error "Khong the cai dat kho EPEL"
    ...
    # Kiem tra cac goi khac
    if ! command -v curl &> /dev/null || ! command -v host &> /dev/null; then
        echo -e "${CYAN}Cai dat curl, gnupg2, va bind-utils...${NC}"
        sudo dnf install -y curl gnupg2 bind-utils
        check_error "Khong the cai dat cac goi can thiet"
    fi
...
install_docker() {
    ...
    if command -v docker &> /dev/null; then
        ...
        # Cai dat dnf-utils de quan ly repository
        ...
        # Cai dat Docker Compose (standalone - de dam bao tuong thich script)
        sudo curl -SL "https://github.com/docker/compose/releases/latest/download/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
        sudo chmod +x /usr/local/bin/docker-compose
        check_error "Khong the cai dat Docker Compose"
...
configure_n8n() {
    ...
    # Luu y: 'docker-compose' thay vi 'docker compose' de chay
...
configure_nginx() {
    ...
    if [ -f "/etc/nginx/conf.d/n8n.conf" ]; then
        ...
        # Sao luu cau hinh cu
        sudo cp /etc/nginx/conf.d/n8n.conf /etc/nginx/conf.d/n8n.conf.bak
    fi
...
install_ssl() {
    ...
    if sudo certbot certificates | grep -q "$DOMAIN"; then
        ...
        if ! grep -q "ssl_certificate" /etc/nginx/conf.d/n8n.conf; then
        ...
        # Cho phep Nginx ket noi mạng (quan trong cho SELinux)
        if sestatus | grep "SELinux status" | grep -q "enabled"; then
             echo -e "${YELLOW}Phat hien SELinux, dang cap phep cho Nginx ket noi mang...${NC}"
...
main() {
    ...
    if [ "$setup_mode" = "1" ]; then
        ...
        read -p "$(echo -e ${CYAN}Nhap email cua ban (dung cho SSL): ${NC})" EMAIL
    ...
    echo -e "- Neu gap loi 502 Bad Gateway, kiem tra SELinux: ${YELLOW}sestatus${NC}. Neu 'enabled', co the can chay: ${YELLOW}sudo setsebool -P httpd_can_network_connect 1${NC} va khoi dong lai Nginx."
